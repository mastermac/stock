...
gulp.task('js-lite-build', function() {
  const plugins1 = getJSLiteModules();

  gulp.src(plugins1.moduleslite)
    .pipe(concat('mdblite.js'))
    .pipe(gulp.dest('./dist/js/'));
...
function getJSLiteModules() {
  delete require.cache[require.resolve('./js/moduleslite.js')];
  return require('./js/moduleslite');
}
