    ...
    var temp = "";
        if (data != null) {
            for (i = 0; i < data.total; i++) {
                ...
                sb = sb.replace("{MU}",parseFloat(GiveValidNumber(data.data[i].mu)).toFixed(2));
                sb = sb.replace("{CostPrice}","$ "+NullCheckNA(data.data[i].costPrice));
                sb = sb.replace("{Size}",data.data[i].ringSize);
                sb = sb.replace("{GW}",parseFloat(data.data[i].grossWt).toFixed(2));
                sb = sb.replace("{DiaWt}",parseFloat(data.data[i].diaWt).toFixed(2));
                sb = sb.replace("{CS}",parseFloat(data.data[i].cstoneWt).toFixed(2));
                sb = sb.replace("{Gold}",parseFloat(data.data[i].goldWt).toFixed(2));
